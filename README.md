...
There are three input files: **near_parcels.gpkg**, the output geopackage file from the previous exercise; **tl_2018_36_bg.zip**, a shapefile of Census block groups for New York State; and **class_200_by_bg.csv**, a CSV file giving the count of residential properties (property class 200) in each block group in Onondaga County. The shapefile will need to be downloaded from the Census. Be sure to get the 2018 version for consistency with data that will be downloaded from the API server. If you had trouble with the last exercise, you can download a copy of "near_parcels.gpkg" from the Google Drive folder for this exercise.
...
1. Use the response to construct a dataframe called `pop`.

1. Use a dictionary to rename the columns of `pop` for `"B02001_001E"` and `"B02001_002E"` to `"total"` and `"white"`.

1. Create a new column in `pop` called `"GEOID"` that is the result of concatenating the columns for `"state"`, `"county"`, `"tract"` and `"block group"`. Note that there should be no spaces between the parts.
...
1. Set `bg_data` to the result of using `pd.read_csv()` to read `"pop_by_bg.csv"`. Be sure to use a dictionary to set the `dtype` argument so that `"GEOID"` will be read as a string.

1. Set the index of `bg_data` to `"GEOID"`.

1. Set column `"poc"` in `"bg_data"` equal to the difference between the `"total"` and `"white"` columns of `bg_data`.
...
1. Set `near` to the result of using `geopandas.read_file()` to read `"near-parcels.gpkg"` layer `"parcels"`. These are the residential properties within 2000 m of the interstate.

1. Set `prop` equal to the result of applying the `.astype(float)` method to the `"PROP_CLASS"` column of `near`.

1. Set `is_res` equal to the result of calling the `.between()` method on `prop` with two arguments: `200` and `299`. The result will be a series indicating whether each row of the dataframe is a residential property.
...
1. Set `bg_geo` to the result of using a `.query()` call to filter `bg_geo` down to the block groups where `"COUNTYFP"` is equal to `"067"`.

1. Set `bg_geo` to the result of using the `.to_crs()` method to change the projection for `bg_geo` to `epsg:26918`. As in a previous exercise, that will speed up a subsequent spatial join by matching the projection used in the parcel file.
...
1. Set column `"near"` of `count_res` to the result of applying the `.sum()` method to `near_counts` with the argument `axis="columns"`. 
...
1. Set `houses` to the result of calling `pd.DataFrame()` with the argument `index=count_res.index`. Using the `index=` argument ensures that the index will have an entry for every inhabited block group in the county (not just the ones near the highways) since `county_res` was built from the Census population data.
...
1. Set `stacked` to the result of calling `.stack()` on `shares`. The result will be convenient for a step below where we'll estimate the racial populations of each ring.
